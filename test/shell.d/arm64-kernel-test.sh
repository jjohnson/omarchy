...
OMARCHY_TEST_LIMINE_LOG="$test_tmp/limine.log" \
  "$ROOT/bin/omarchy-update-kernel-arm64"

[[ $(<"$test_tmp/mkinitcpio.log") == "-p linux-aarch64" ]] ||
  fail "ARM64 updater rebuilds the linux-aarch64 preset"
pass "ARM64 updater rebuilds the linux-aarch64 preset"
...
[[ $(<"$test_tmp/limine.log") == "$expected_limine_args" ]] ||
  fail "ARM64 updater creates a native Limine Linux entry"
pass "ARM64 updater creates a native Limine Linux entry"
...
OMARCHY_GRAPHICS_SYSFS="$test_tmp/graphics" \
  source "$ROOT/install/hardware/arm64-kernel.sh"

cmp \
  "$ROOT/install/hardware/99-omarchy-arm64-kernel.hook" \
  "$hook_root/etc/pacman.d/hooks/99-omarchy-arm64-kernel.hook" ||
  fail "ARM64 hardware setup installs the kernel maintenance hook"
pass "ARM64 hardware setup installs the kernel maintenance hook"

[[ $(<"$hook_root/etc/limine-entry-tool.d/arm64-virtio-plymouth.conf") == \
  'KERNEL_CMDLINE[default]+=" plymouth.ignore-serial-consoles"' ]] ||
  fail "ARM64 hardware setup keeps Plymouth graphical in VirtIO guests"
pass "ARM64 hardware setup keeps Plymouth graphical in VirtIO guests"
...
(( ${#MODULES[@]} == 0 )) ||
  fail "mkinitcpio skips the unavailable ARM64 thunderbolt module"
pass "mkinitcpio skips the unavailable ARM64 thunderbolt module"
