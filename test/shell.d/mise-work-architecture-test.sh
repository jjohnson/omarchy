...
run_architecture_case() {
  ...
  local case_dir="$test_tmp/$architecture"
...
MISE
  ...
  [[ -x $test_home/.local/share/mise/installs/node/$version/bin/node ]] ||
    fail "$architecture extracts the bundled Node.js archive"
  grep -qxF "use -g node@$version" "$mise_log" ||
    fail "$architecture activates the bundled Node.js version"
  pass "$architecture installs the bundled linux-$node_arch Node.js archive"
}

run_architecture_case x86_64 x64
run_architecture_case aarch64 arm64
