...
case ${OMARCHY_SETUP_CONTEXT:-runtime} in
  iso-chroot) NODE_PACKAGE_DIR="${OMARCHY_PACKAGES_DIR:-/opt/packages}" ;;
  provision-owner) NODE_PACKAGE_DIR="${OMARCHY_PACKAGES_DIR:-/var/lib/omarchy/provisioning/packages}" ;;
  *) NODE_PACKAGE_DIR="" ;;
esac
...
if [[ -n $NODE_PACKAGE_DIR ]]; then
  case ${OMARCHY_ARCH:-$(uname -m)} in
    ...
    *)
      echo "Error: unsupported Node.js architecture ${OMARCHY_ARCH:-$(uname -m)}" >&2
      exit 1
      ;;
  esac
  ...
  if [[ -z $NODE_TARBALL ]]; then
    if [[ ${OMARCHY_SETUP_CONTEXT:-} == "provision-owner" ]]; then
      ...
      echo "Error: bundled Node.js linux-${NODE_ARCH} tarball missing from $NODE_PACKAGE_DIR" >&2
      exit 1
    fi
  else
