...
if [[ $architecture == "aarch64" || $architecture == "arm64" ]]; then
  ...
  install -Dm644 \
    "$OMARCHY_INSTALL/hardware/99-omarchy-arm64-kernel.hook" \
    "$system_root/etc/pacman.d/hooks/99-omarchy-arm64-kernel.hook"

  plymouth_dropin="$system_root/etc/limine-entry-tool.d/arm64-virtio-plymouth.conf"
  legacy_console_dropin="$system_root/etc/limine-entry-tool.d/arm64-virtio-console.conf"
  rm -f "$legacy_console_dropin"
